...
## 프로젝트 개요
최신 초거대 언어 모델을 내 비즈니스나 도메인에 꼭 맞게 맞춤형으로 미세조정(파인튜닝)해주는 "글로벌 표준 AI 맞춤 학습 키트"
복잡한 분산 학습 설정 없이 간단한 설정 파일 하나로 모델의 두뇌를 내 전문 분야 지식으로 완벽히 재교육
기업 전용 프라이빗 AI 모델을 만들거나 독창적인 특화 모델을 출시하고 싶은 팀을 위한 최적의 조련 도구
